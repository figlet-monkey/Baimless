...
namespace notifies
{
	...
	void show_block( const notify_data_t& notify)
	{
		float alpha = 1.f;
		...
		g::notify->AddRectFilled(block_pos, notify_end_pos, alpha_modulate(background_color, alpha), 2.f);
		...
		g::notify->AddRectFilled(block_pos, ImVec2(block_pos.x + 5.f, block_pos.y + notify_size.y), alpha_modulate(color, alpha), 1.f);

		const auto text_pos = ImVec2(block_pos.x + 5.f + 10.f, block_pos.y + notify_size.y / 2.f - text_size.y / 2.f);

		g::notify->AddText(text_pos, alpha_modulate(text_color, alpha), text_size.x > notify_size.x + 10.f ? notify.message.substr(0, 25).c_str() : notify.message.c_str());

		block_pos.y -= notify_size.y + 10.f;
	}

	void show_text(const notify_data_t& data)
	{
		ImGui::PushFont(notifies_font);
		g::notify->AddText(debug_pos, ImGui::GetColorU32(ImVec4(255,255,255,255)), data.message.c_str());
		ImGui::PopFont();
